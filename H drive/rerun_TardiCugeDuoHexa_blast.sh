...
## Temporarily replaced by file size check. This does mean I need to check all files but now I can make sure I have output for all of them
#if test -d ~/h/BLAST/TardiCugeDuoHexa_Blast/"$strain_name"_res/; then
...
#mkdir ~/h/BLAST/TardiCugeDuoHexa_Blast/"$strain_name"_res/
for gene in ~/h/BLAST/TardiCugeDuoHexa_Blast/*_genes/*.fasta
do
	gene_name=$(echo "$gene" | rev | cut -d "/" -f 1 | rev | cut -d "." -f1)
	touch ~/h/BLAST/TardiCugeDuoHexa_Blast/"$strain_name"_res/"$gene_name"."$strain_name".out
	if [ $(wc -c < ~/h/BLAST/TardiCugeDuoHexa_Blast/"$strain_name"_res/"$gene_name"."$strain_name".out) -lt 1500 ]; then
		echo "BASTing for $gene"
		blastn -subject "$input_file" -query "$gene" -out ~/h/BLAST/TardiCugeDuoHexa_Blast/"$strain_name"_res/"$gene_name"."$strain_name".out 
	fi	
done
echo " " 
