...
#	       ~/h/scripts/tel_name_parser

seq_name=$(echo "$1" |cut -d "/" -f 3 | cut -d "." -f 1) #Cuts the name of the german sequences shorter
telo_file=$(~/h/scripts/telo_name_parser.sh "$1")

if ! test -f ~/h/Metadata_sequences/sequence_info.csv; then  #Checks is the file already exists and creates it with a header if it doesn't
	echo "File does not exist yet"
	echo Strain,Contigs,Scaffolds,Length,GC_content,N50,L50,Telomeres,Telomere_matches,Busco_completeness,Percent_gaps >~/h/Metadata_sequences/sequence_info.csv
fi
...
if ! grep "$seq_name" ~/sequence_info.csv -q; then   #Checks if a strain name is already present in csv,
	...
	busco_stats=$(~/h/scripts/busco_summary_extract.sh "$seq_name")
	...
	N50=$(python ~/h/scripts/N50_calculator.py "$1")
	L50=$(python ~/h/scripts/L50_calculator.py "$1")
	...
	echo "$seq_name","$no_contigs","$no_scaffolds","$total_length","$GC_content""%","$N50","$L50","$num_of_telomeres","$matched_telomeres","$busco_completeness","$percent_gaps" >> ~/h/Metadata_sequences/sequence_info.csv
fi
