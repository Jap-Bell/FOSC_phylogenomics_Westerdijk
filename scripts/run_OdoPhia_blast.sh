...
# I want this script to take an assembly as input ($1), build a database from this with a given name structure, blast the 6 Odo+Phia genes on it, and view the output
...
if test -d ~/h/BLAST/PurOdoPhia_Blast/"$strain_name"_res/; then
...
mkdir ~/h/BLAST/PurOdoPhia_Blast/"$strain_name"_res/
for gene in ~/h/BLAST/PurOdoPhia_Blast/*_genes/*.fasta
	do
	gene_name=$(echo "$gene" | rev | cut -d "/" -f 1 | rev | cut -d "." -f1)
	touch ~/h/BLAST/PurOdoPhia_Blast/"$strain_name"_res/"$gene_name"."$strain_name".out
	blastn -subject "$input_file" -query "$gene" -out ~/h/BLAST/PurOdoPhia_Blast/"$strain_name"_res/"$gene_name"."$strain_name".out 
	done
